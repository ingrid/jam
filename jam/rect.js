define(["exports", "./util", "./lib/sylvester"], function (exports, _util, _sylvester) {
			"use strict";
			...
			var packRecursive = function packRecursive(obj_or_list) {
						var ret = [];
						var list = [];
			...
			self.overlap = function (s1, s2, callback) {
						...
						for (var i = 0; i < g1.length; ++i) {
									for (var j = 0; j < g2.length; ++j) {
												var wsOffset1 = g1[i].getTransform().x(_sylvester2.default.$V([0, 0, 1]));
												var wsOffset2 = g2[j].getTransform().x(_sylvester2.default.$V([0, 0, 1]));
			...
			self.separate = function (s1, s2) {
						...
						if (s1.immovable && s2.immovable) {
									return;
						} else if (s1.immovable) {
									staticCo2 = 1.0;
									staticCo1 = 0.0;
						} else if (s2.immovable) {
									staticCo1 = 1.0;
									staticCo2 = 0.0;
						} else {
									staticCo1 = 0.5;
						...
						var wsOffset1 = s1.getTransform().x(_sylvester2.default.$V([0, 0, 1]));
						var wsOffset2 = s2.getTransform().x(_sylvester2.default.$V([0, 0, 1]));
